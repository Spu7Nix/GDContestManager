...
module.exports = {
  ...
  execute: async function(client, message, words) {
    ...
    if (words.length > 2) { // !award [usertag] [amount]
      ...
      for (let i = 0; i < 5; i++) {
        user = await client.fetchUser(contests[c].top5[i])
        await givePoints(user, awardedPoints[i], client)
        message.channel.send(`Gave ${awardedPoints[i]} points to ${user.username}`)
      }
